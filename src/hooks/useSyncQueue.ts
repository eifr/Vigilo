...
export function useSyncQueue(options: UseSyncQueueOptions) {
  ...
  const { telegramBotToken, telegramChatId, maxRetries = 3, syncBatchSize = 5 } = options;
  ...
  const sendTelegramMessage = useCallback(
    async (frame: string, originalTimestamp?: number): Promise<boolean> => {
      try {
        ...
        if (!response.ok) {
          ...
          if (
            errorData.description?.includes("photo") ||
            ...
            const smallerBlob = await dataUrlToBlob(
              frame.replace(/quality=[0-9.]+/, "quality=0.3"),
            );
            ...
            const fallbackTextResponse = await fetch(
              `https://api.telegram.org/bot${telegramBotToken}/sendMessage`,
              {
                method: "POST",
                headers: {
            ...
            throw new Error(
              `Telegram API error: ${response.status} - ${errorData.description || "Unknown error"}`,
            );
          }
        }
  ...
  const tryImmediateSend = useCallback(
    async (frame: string, originalTimestamp: number): Promise<boolean> => {
      if (!isOnline) {
        return false;
      }
  ...
  const queueEventOffline = useCallback(
    async (deviceId: string, frame: string, timestamp: number, metadata?: any): Promise<string> => {
  ...
  const syncEvent = useCallback(
    async (event: SyncQueueItem): Promise<boolean> => {
      try {
      ...
      for (const event of pendingEvents) {
        ...
        try {
          ...
          if (success) {
            ...
            await offlineStorage.updateEventStatus(
              event.id,
              "failed",
              "Failed to send during flush",
            );
          }
  ...
  const syncEvents = useCallback(
    async (events: SyncQueueItem[]): Promise<number> => {
