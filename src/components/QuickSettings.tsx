...
import { Settings, RefreshCw, Shield, AlertTriangle, Clock, Zap, Eye } from "lucide-react";
  ...
  return (
    <div className={`p-3 rounded-lg border ${getStatusBg()}`}>
      <div className="flex items-center justify-between">
        ...
        {value && <span className={`text-lg font-bold ${getStatusColor()}`}>{value}</span>}
      </div>
    </div>
  );
}
...
export function ActionButton({
  ...
  return (
    <Button variant={variant} onClick={onClick} disabled={disabled || loading} className="w-full">
      {loading ? (
      ...
      <CardContent className="space-y-6">
        <div className="space-y-2">
          ...
          <Input
            ...
            step="500"
          />
          <p className="text-xs text-muted-foreground">Minimum time between motion alerts</p>
        </div>
