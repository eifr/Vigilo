...
export function OfflineStatusBanner({
  ...
  const getStatusText = () => {
    ...
    if (syncStatus.isFlushing)
      return `You were offline. Syncing ${syncStatus.pendingCount} missed events...`;
    if (syncStatus.isSyncing)
      return `Syncing... (${syncStatus.syncProgress}/${syncStatus.totalToSync})`;
    if (syncStatus.failedCount > 0) return `${syncStatus.failedCount} Failed Events`;
    if (syncStatus.pendingCount > 0 && syncStatus.wasOffline)
      return "Back Online - Queued events ready";
  ...
  const getSubtext = () => {
    ...
    if (syncStatus.isFlushing)
      return `Sending missed events from offline period (${syncStatus.syncProgress}/${syncStatus.totalToSync})`;
    ...
    if (syncStatus.pendingCount > 0 && syncStatus.wasOffline)
      return `${syncStatus.pendingCount} events will be sent when connection is stable`;
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                {getSubtext() && <div className="text-sm opacity-75">{getSubtext()}</div>}
              </div>
            </div>

            <div className="flex items-center gap-2">
              {syncStatus.failedCount > 0 && onRetrySync && (
                <Button variant="outline" size="sm" onClick={onRetrySync} className="text-xs">
              ...
              {syncStatus.failedCount > 0 && onClearFailed && (
                <Button variant="outline" size="sm" onClick={onClearFailed} className="text-xs">
                  Clear Failed
                </Button>
              )}
  ...
  if (isOnline) {
    return (
      <div className={`flex items-center gap-2 text-green-600 ${className}`}>
        ...
        {effectiveType && <span className="text-xs opacity-75">({effectiveType})</span>}
      </div>
    );
  }
